...
@section('content_header')
    <div class="d-flex justify-content-between mx-4">
        <h1>Drinks Table</h1>
        <div>
            <form action="{{ route('accessory.create') }}" method="GET" style="display: inline;">
...
@section('content')
    <div class="">
        ...
        <div class="card-body">
            <table id="example" class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($accessorys as $index)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $index->name }}</td>
                            <td>{{ ucwords($index->brand) }}</td>
                            <td>
                                <img src="{{ asset('' . $index->image) }}" alt="Drink Image" style="max-width: 100px; max-height: 100px;">
                            </td>
                            <td>{{ ucwords($index->price) }}</td>
                            <td>
                                <a href="{{ route('accessory.edit', $index->id) }}" class="btn btn-secondary btn-sm">Update</a>
                                <!-- Delete Form -->
                                <form action="{{ route('accessory.destroy', $index->id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="pagination-container mt-5 mb-5 d-flex justify-content-end">
        {{ $accessorys->links('pagination::bootstrap-4') }}
    </div>
@stop
